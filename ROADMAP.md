...
- 🚧 [ANTS-1428] **Adapter mode for non-Ants ROADMAP formats — read + write across MCP and Roadmap dialog.** Tier 1 (read-side adapter) shipped 2026-05-16 (Bundle C pull 13). Spec: [`docs/specs/ANTS-1428.md`](docs/specs/ANTS-1428.md). `RoadmapDialog::parseBullets` now detects GFM-task-list shape (no `<!-- ants-roadmap-format: 1 -->` marker + `^- \[[ x]\]` bullets in first 100 non-empty lines) and engages the adapter branch: `- [x]` → ✅, `- [ ]` → 📋, inline-emoji prefix wins over checkbox state, `**Bold-ID.**` tokens preserved (multi-prefix projects like Vestige's `Sh`/`Ed`/`VEST` work without configuration), synthetic IDs via FNV-1a 64-bit content-hash of the normalised headline (stable across line reorders, ~10⁻¹⁴ collision probability at document scale), `(COMPLETE)`/`(DONE)` heading marker causes enclosed planned bullets to inherit ✅, caret anchors (`^vest-0042`) extracted as future locator handles. Envelope echoes `format:"github-task-list"` + per-bullet `synthetic:true` + `anchor:"..."` when in adapter mode. Vestige's roadmap (`/mnt/Games/Scripts/Linux/3D_Engine/ROADMAP.md`, 402 KB, ~600 bullets) now parses end-to-end through `roadmap_query`. Tests: `tests/features/mcp_adapter_github_tasklist/` (10 INVs; 756/756 features green at landing). **Remaining for next bundle**: Tier 2 (write-side `op:"flip"` with caret-anchor injection + `.roadmap-counter` consume-on-write semantics) and Tier 3 (dialog renderer fork for the v2 card UI). Spec covers all three tiers; ship state advances 📋→🚧.
