...
| `Inv11FailOpenWiring` | INV-11 — `offloadBody` returns the body on `ensurePrivateDir`/`commit` failure, and removes a spill it cannot make owner-only (ANTS-5104). |
| `Ants5104PreviewLadderIsLinearPerRung` | ANTS-5104 — the `rows_preview` ladder builds each row's JSON once and sums row lengths per rung, rather than re-serialising the growing array for each row. Source scrape; the preview's content stays pinned by the ANTS-4397/4474/4519 cases. |
