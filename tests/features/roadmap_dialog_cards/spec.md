...
- `renderCardsHtml` emits the load-bearing HTML shape — one
  `<tr class="rm-card">` per top-level status-emoji bullet inside a
  per-section `<table class="rm-cards">`, with aligned state / kind /
  summary / meta columns and an expand toggle in the meta cell
  (ANTS-3392).
...
| #  | Lane     | Statement |
|----|----------|-----------|
| 1  | renderer | `renderCardsHtml` emits each top-level status-emoji bullet as a `<tr class="rm-card" id="rm-ANTS-NNNN">` inside a per-section `<table class="rm-cards">` (ANTS-3392; pre-3392 this was a `<div class="rm-card">`). Anchor: `// ANTS-1154-INV-1` on the line preceding the `rm-card` row literal. |
| 2  | renderer | Each `rm-card` row carries four `<td class="rm-col-*">` cells: **state** (`<span class="rm-state">` + `<span class="rm-state-label">`), optional **kind** (`<span class="rm-kind">`), **summary** (`<span class="rm-summary">`), and **meta** (`<span class="rm-id">`, optional `<span class="rm-date">`, then `<a class="rm-toggle" href="ants://expand/<id>">` — or `ants://collapse/<id>` when expanded). (ANTS-3392) |
...
| 16 | renderer | The expanded body of each card MUST NOT be wrapped in a `<div class="rm-body">` block. Under ANTS-3392 the body is emitted as a following `<tr class="rm-card-body">` whose single `<td colspan="4">` cell contains `<p class="rm-body-first">` (the first non-empty body line, carrying the dotted divider + extra top padding) then `<p class="rm-body-line">` (subsequent lines, indent only). Qt's text engine renders a nested `<div>` block with its own `QPalette::Base` background frame, so the prior `<div class="rm-body">` wrapper painted a different colour than the parent card; the colspan cell inherits the card background instead. (ANTS-1240 / ANTS-3392) |
| 17 | renderer | The card's hashed ID (`#NNNN`) and date (when present) MUST be emitted as `<span class="rm-id">` / `<span class="rm-date">` inside the row's `<td class="rm-col-meta">` cell, with the `rm-id` span **before** the `rm-toggle` anchor — NOT wrapped in a `<div class="rm-meta">`. Same Qt nested-block QPalette::Base rationale as INV-16: the prior `<div class="rm-meta">` rendered a darker band under the ID on dark themes. The `.rm-id` CSS rule MUST set `font-size:12px` at the Cozy tier (parameterised via the `codePx` slot) so the number is scannable at a glance. (ANTS-1241 / ANTS-3392) |
...
| 19 | renderer | The lookup table `kStatusLabels` lives at file scope in `roadmapdialog.cpp` and has exactly five entries (🚫 added by ANTS-4977), asserted by `static_assert(std::size(kStatusLabels) == 5)`. The runtime backstop is INV-18 — a fifth status emoji with no table entry makes `statusAccessibleLabel` return empty, the rm-state-label span is skipped, INV-18 fails. (ANTS-1235) |
