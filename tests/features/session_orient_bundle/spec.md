...
| 6b  | `Ants3587AbsentRoadmapKeepsOkAddsNotice` | an upstream that refuses ONLY for an absent optional artifact (`no_roadmap_loaded`/`no_roadmap`) keeps top-level `ok:true` and is surfaced in a `notices[]` array, not `ok:false`; real failures (bad cwd, unreadable state) still fail (ANTS-3587). |
...
| 9   | `Inv9FeedbackPendingScan`         | bundle surfaces the cross-session feedback backlog under a `feedback_pending` key, filled from `RemoteControl::buildFeedbackPendingBlock`, which reuses `FeedbackFile::parse`, is gated to the maintainer project by `docs/standards/mcp-feedback-files.md`, and surfaces only files with an un-triaged `deltaPresent` (ANTS-1964; extracted by ANTS-4896 so the scan is testable without a MainWindow). |
...
| 11  | `Inv11ServerBuildStaleFlag`       | bundle actively flags a stale server binary: compares `ANTS_BUILD_COMMIT` against project HEAD via `git rev-list --count <build>..HEAD` and, when behind, emits a `server_build_stale` block with `behind_commits`; advisory-only (never fails the bundle) (ANTS-3499). |
