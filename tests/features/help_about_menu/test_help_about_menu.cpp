...
//                ANTS_BUILD_TYPE, and ANTS_BUILD_COMMIT (extern symbols
//                declared in the stable src/build_info.h; ANTS-3582).
...
TEST(HelpAboutMenu, Main) {    auto loadFile = [](const QString &path) -> QString {
    ...
    // I11 — header inclusion. src/build_info.h (ANTS-3582: a stable
    // hand-written extern-declaration header) declares the ANTS_BUILD_*
    // symbols; the definitions live in the generated build_info_values.cpp.
    // A dropped #include would fail to link rather than silently miss.
    expect(aboutSrc.contains(QStringLiteral("#include \"build_info.h\"")),
           ...
           QStringLiteral("aboutdialogs.cpp must #include \"build_info.h\" "
                          "(the stable extern-decl header; see ANTS-3582)."));

    // Invariant 12 — ANTS-1371: the Donate menu carries a
    // "Sponsor on GitHub" action whose handler opens the canonical
    // GitHub Sponsors URL via QDesktopServices. The original request
    // (2026-05-14) asked for a standalone "Sponsors" menu entry after
    // Help; the 2026-06-30 Donate menu subsumes it (Donate is the
    // rightmost menu, and "Sponsor on &GitHub..." is its first action).
    // This invariant locks the content contract the request cared about:
    // one in-GUI click reaches https://github.com/sponsors/milnet01,
    // kept lockstep with .github/FUNDING.yml (`github: [milnet01]`).
    const QString donateBlock = extractFunctionBody(
        src, QStringLiteral("void MainWindow::setupDonateMenu()"));
    expect(!donateBlock.isEmpty(), "I12a/donate-block-located",
           QStringLiteral("Could not locate setupDonateMenu() body in "
                          "mainwindow.cpp — has the function been renamed "
                          "or removed?"));
    expect(donateBlock.contains(QStringLiteral("Sponsor on &GitHub...")),
           "I12b/sponsor-github-action-present",
           QStringLiteral("Donate menu must expose a 'Sponsor on GitHub' "
                          "action (ANTS-1371) — the in-GUI path to the "
                          "GitHub Sponsors page."));
    expect(donateBlock.contains(
               QStringLiteral("https://github.com/sponsors/milnet01")),
           "I12c/sponsor-url-canonical",
           QStringLiteral("The Sponsor-on-GitHub handler must open "
                          "https://github.com/sponsors/milnet01 — the "
                          "canonical destination mirrored in "
                          ".github/FUNDING.yml. A drift here breaks the "
                          "in-app funding link."));
    expect(donateBlock.contains(QStringLiteral("QDesktopServices::openUrl")),
           "I12d/opens-via-qdesktopservices",
           QStringLiteral("The action must open the URL via "
                          "QDesktopServices::openUrl (the Qt 6 idiom that "
                          "dispatches to xdg-open / the platform handler)."));
