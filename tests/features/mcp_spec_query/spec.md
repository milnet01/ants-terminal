...
| 3 | The body validates the `id` argument against `^ANTS-[0-9]+$` (rejects malformed ids with `code:"bad_id"`). |
...
| 7 | The `tools/list` block in `src/claudeintegration.cpp` registers a `"spec_query"` entry. Schema declares `properties.id` (required) and `properties.caller_cwd` (required). |
