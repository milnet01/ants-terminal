        ...
        connect(socket, &QLocalSocket::readyRead, this, [this, socket, idleTimer]() {
            ...
            if (method == "initialize") {
                ...
                rsTool["description"] = QStringLiteral(
                    ...
                    "what to fetch. ANTS-4519: when a text sample cannot fit "
                    "for every row, `rows_preview` is OMITTED rather than sent "
                    "headless (a bare list of row LENGTHS says no more than "
                    "`row_count` + `bytes`); `rows_preview_omitted:true` marks "
                    "that case. `head_rows` is unchanged and still the right "
                    "preview for many small rows. "
