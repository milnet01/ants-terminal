            ...
            if (method == "initialize") {
                ...
                auto makeFieldsProp = []{
                    ...
                    p["description"] = QStringLiteral(
                        ...
                        "\"files\"]). A name the envelope does not carry is "
                        "listed in `fields_unmatched` (ANTS-4567), emitted only "
                ...
                roadmapTool["detail"] = QStringLiteral(
                    ...
                    "ANTS-4575 — WHETHER AN ID WAS WRITTEN DOWN. A bullet "
                    "carries `id_inferred:true` when the reader ADOPTED its "
                    "id from a bold prose lead-in instead of reading a "
                    "bracket token the author wrote: `- [ ] **Terrain "
                    "System** — …` is reported with id \"Terrain System\". "
                    "Gated, so a roadmap whose ids are all bracketed sends "
                    "the same bytes as before. Two things it does NOT say. "
                    "It is not `synthetic`, which flags a content-hash id "
                    "manufactured when there was no token at all — the two "
                    "never both fire. And it describes THE TEXT THIS CALL "
                    "READ: a migrated project's ROADMAP.md is rendered from "
                    "the store with every id in brackets, so the flag goes "
                    "quiet there rather than reporting on how the id was "
                    "first obtained. Nothing in the text separates an "
                    "inferred id from a real multi-word one, which is why "
                    "two bullets sharing a lead-in silently collide; "
                    "ANTS-3771 (declare the project's id format) is the "
                    "fix for that, and this flag is only the warning. "
                    ...
                    "saw the same id on more "
                    "than one bullet (each entry: `{id, occurrences:"
                    "[{section_slug, status}], truncated_count?}`). "
                    "ANTS-4546 — an AUTHORED id counts whatever its "
                    "shape: on a GFM roadmap two bullets leading with "
                    "the same bold span (`**Photo mode**`) BOTH carry "
                    "that id, and roadmap_log then refuses "
                    "`bullet_ambiguous` — so a silence here is paid for "
                    "at write time. Only the reader's own synthetic "
                    "content-hash nonces are excluded (ANTS-1688, where "
                    "they over-reported); `occurrences[]` "
                    "is capped at 3 with the dropped tail in "
                    "`truncated_count`. Absent when the roadmap is "
                    "clean. Read it as \"this id addresses more than one "
                    "bullet\" — which covers both hand-edited drift past "
                    "the `.roadmap-counter` guard and an inferred id two "
                    "bullets happen to share. "
                    ...
                    "ANTS-4604 — MIXED-DIALECT ROADMAPS. `format` here means "
                    "\"the GFM adapter engaged, so kind/lanes/layman are "
                    "degraded\" — it is NOT a claim that the whole file is one "
                    "dialect, and roadmap_log's `format` answers a different "
                    "question again (the dialect of the bullet it touched, or "
                    "the one it wrote). On a file carrying BOTH shapes the "
                    "envelope adds `mixed:true` with `gfm_bullets` and "
                    "`non_gfm_bullets` counts, so a caller keying on "
                    "`format != \"ants-v1\"` can see it is not looking at a "
                    "uniform file. All three are ABSENT on a uniform roadmap, "
                    "so an envelope without them is not an older server. "
                    "`non_gfm_bullets` is deliberately not called a dialect: "
                    "the per-bullet tag is emitted only for github-task-list, "
                    "so the remainder is everything else. None of this decides "
                    "which backend serves the project — that is "
                    "migratedProject()'s own detector, which reads neither "
                    "field. ANTS-4606 — the two verbs also SPELL that dialect "
                    "differently: this verb reports `github-task-list` where "
                    "roadmap_log reports `gfm`. One format, two vocabularies, "
                    "so a caller comparing the two envelopes reads a "
                    "disagreement that is not one. Both spellings stand "
                    "deliberately: unifying them would silently break any "
                    "caller already keying on either, and this store is "
                    "machine-global, so that caller may be another project's "
                    "session. Compare dialects by mapping both spellings, "
                    "never by string equality across the two verbs.");
                    ...
                    "for full-text edits.");
                {
                    ...
                    inclBodyProp["description"] = QStringLiteral(
                        ...
                        "ANTS-4557 — `body` is the bullet's CONTINUATION "
                        "lines only: the head line is `headline` / "
                        "`headline_oneline` in the same envelope and is no "
                        "longer repeated as the body's first line. The "
                        "trailer lines ARE body lines and stay, on both "
                        "backends. ANTS-4599 — and a trailer line may be "
                        "COMPOSED rather than quoted: the record is built by "
                        "rendering the item and re-parsing it, and the render "
                        "emits a line for every column whose key the stored "
                        "prose does not declare at a line start. So a body "
                        "that never wrote `Lanes:` reads back carrying one. "
                        "This field answers WHAT THIS BULLET SAYS IN "
                        "ROADMAP.md — truthfully, since the file is generated "
                        "from the store — and NOT whether the stored body "
                        "declares that field. ANTS-4813 — "
                        "`composed_trailers` names the keys the render wrote, "
                        "so the two ARE distinguishable from the reply. Read "
                        "it before passing a trailer line back as "
                        "op:\"amend_body\" old_text: a key named there is a "
                        "COLUMN, reachable only by op:\"amend_field\", and "
                        "amend_body refuses on it however plainly it appears "
                        "in `body`. Emitted only when non-empty and only "
                        "alongside a body, so its absence is not a withheld "
                        "answer. On the markdown backend it is always empty, "
                        "and that is the TRUE answer rather than a divergence: "
                        "there every line was written by an author, so nothing "
                        "was composed. "
                        "ANTS-4558 — and they keep their own "
                        "indentation: the block is dedented by its COMMON "
                        "leading whitespace, so a nested list or an indented "
                        "command line comes back with its depth, and text you "
                        "read here can be handed straight back to "
                        "op:\"amend_body\". "
                    ...
                    queryProp["description"] = QStringLiteral(
                        ...
                        "combine with id / ids / mode:bundles (bad_mode_combo "
                        "— those are targeted or aggregate surfaces with no "
                        "per-bullet list to filter). ANTS-4610: "
                        "mode:section_index is the EXCEPTION — there `query` "
                        "narrows the INDEX by section name, matching the "
                        "headline or the slug (`query:\"dev experience\"` and "
                        "`query:\"dev-experience\"` both resolve), and the "
                        "reply carries `sections_considered` / "
                        "`sections_filtered_out` so zero matches is "
                        "distinguishable from a roadmap with no sections. It "
                        "is the cheap way to resolve the one slug "
                        "roadmap_log's `section` wants: unnarrowed that index "
                        "runs to tens of rows, and neither `fields` (top-level "
                        "keys, so [\"sections\"] keeps the array) nor "
                        "`compact` (drops empty values, not rows) cuts it. "
                        "ANTS-4367: a SHORT query needs narrowing — "
                    ...
                    // skip the re-emit on sections that didn't change.
                    {
                        ...
                        p["description"] = QStringLiteral(
                            ...
                            "ANTS-4732 — it is the `section` ARGUMENT that "
                            "selects this, never `mode`: the short-circuit "
                            "returns from inside the section branch before "
                            "any projection runs, so it IS honoured under "
                            "mode:\"headline_only\" (this row previously said "
                            "it was ignored there). mode:\"section_index\" and "
                            "mode:\"bundles\" take their own branches and "
                            "never reach it. The etag keys on the section's "
                            "BYTES and not on `mode`, so a caller that varies "
                            "`mode` between calls can be answered `unchanged` "
                            "for a payload shape it did not ask for.");
                ...
                wsTool["detail"] = QStringLiteral(
                    ...
                    "regardless. ANTS-4753 — a `glob` naming a hidden "
                    "path while this is off returns a zero carrying "
                    "`hidden_paths_skipped:true` and a hint: ripgrep "
                    "prunes the directory during the walk, so the empty "
                    "result says nothing about the pattern. A `lane` "
                    "naming one is descended anyway and does not flag), "
                    "dedup (default true — collapses "
                    ...
                    "hunts, 3 Grep calls can still be cheaper.");
                {
                    ...
                    QJsonObject globProp;     globProp["type"]     = "string";
                        ...
                        QStringLiteral("Ripgrep --glob filter (e.g. \"*.cpp\"). "
                                       ...
                                       "ANTS-4672: ONE pattern, never an array "
                                       "— name several paths with a brace "
                                       "group, \"docs/{coding,testing}.md\". "
                                       "`exclude_glob` takes an array and this "
                                       "does not; that asymmetry is ripgrep's "
                                       "own shape, and a brace group works in "
                                       "both.");
                ...
                foTool["description"] = QStringLiteral(
                    ...
                    "literals shows those regions instead of a gap. "
                    "ANTS-4520: in md mode a heading inside a BLOCKQUOTE "
                    "(`> ## You are here`) is a heading and is listed, at the "
                    "depth its `#` run gives it. Anything "
                ...
                // ANTS-4398 — mutation_probe.
                {
                    ...
                    mp["description"] = QStringLiteral(
                        ...
                        "mutant \"dying\" proves nothing then. ANTS-4401: that "
                        "gate needs EVIDENCE of green, not merely the absence "
                        "of red — it also refuses `baseline_unreadable` when "
                        "the baseline exits 0 but its counts come back "
                        "unparsable (-1/-1) or empty (0/0), because a run that "
                ...
                rrTool["description"] = QStringLiteral(
                    ...
                    "(ANTS-4735: brace matching is for BRACE-DELIMITED "
                    "languages only. In py mode the extent comes from the flat "
                    "outline -- the next symbol's line -- because `{` opens "
                    "nothing in Python, and brace-matching there closed on the "
                    "first dict literal and returned a short body while "
                    ...
                    "reads too).");
                {
                    ...
                    QJsonObject sectionProp; sectionProp["type"] = "string";
                        sectionProp["description"] = QStringLiteral(
                            ...
                            "(`> ## You are here`, the standing convention for "
                            "run-state blocks in a plan file) is a heading and "
                            "resolves by name like any other; its body ends at "
                            "the next heading that is less deeply quoted, or "
                            "equally quoted at the same-or-higher level, so a "
                            "quoted block cannot swallow the plain section "
                            "after it. Mutually exclusive with the other "
                ...
                rsTool["description"] = QStringLiteral(
                    ...
                    "what to fetch. ANTS-4474: when a full-width sample will "
                    "not cover every row the head is NARROWED before it is "
                    "dropped — the width steps 60 → 40 → 24 → 12 and the "
                    "narrowest that covers them all is used, reported as "
                    "`rows_preview_head_chars` (present only when below the "
                    "60 default). A row label, a Markdown heading, a table "
                    "cell name and a function signature all live in the first "
                    "~40 characters, so a narrowed head still says WHAT a row "
                    "is — the one question `bytes` cannot answer. ANTS-4519: "
                    "only when NO width covers every row is `rows_preview` "
                    "OMITTED rather than sent headless (a bare list of row "
                    "LENGTHS says no more than `row_count` + `bytes`); "
                    "`rows_preview_omitted:true` marks that case. `head_rows` "
                    "is unchanged and still the right preview for many small "
                    "rows. "
                ...
                aeTool["description"] = QStringLiteral(
                    ...
                    "ANTS-4733: a TRANSPORT TIMEOUT means UNKNOWN, never "
                    "failed. Measured -- a batch reported a transport "
                    "timeout, a check of the files immediately afterwards "
                    "showed NOTHING written, and the write then landed. So "
                    "the obvious recovery (look, see no change, re-send) "
                    "races a write still in flight. RE-READ before deciding, "
                    "and prefer the unique-`old` form: its not_found refusal "
                    "is what makes a duplicate re-send safe, and neither "
                    "`replace_all` nor a start_line/end_line range has an "
                    "equivalent guard. "
                    "ANTS-4418: a `not_found` skip whose text differs from the "
                    "file ONLY in spacing carries `candidates:[{line,text}]`, "
                    "`near_miss_line` and a `hint` — same field names as "
                    "read_region section-mode and roadmap_log's locators, so "
                    "one caller path handles all three. Retry with that line's "
                    "verbatim `text`, or with the start_line/end_line form for "
                    "that line; do NOT re-read the file and guess the spacing "
                    "again, which is the round-trip this exists to remove. "
                    "Reported only when the near miss is UNIQUE and "
                    "whitespace-class, and only for a single-line `old`. "
                    "ANTS-4473: an `ambiguous` skip carries the SAME "
                    "`candidates:[{line,text}]` naming where the occurrences "
                    "are, plus `match_count` (the true total) and a `hint`. "
                    "The two cases are mutually exclusive, so one caller path "
                    "covers both. The list is capped at 10 and sets "
                    "`candidates_truncated` when it bites — `match_count` is "
                    "always the real number. Recover by extending `old` with "
                    "surrounding context, by using the start_line/end_line "
                    "form for the occurrence you mean, or with "
                ...
                ciTool["description"] = QStringLiteral(
                    ...
                    "file_outline coverage. ANTS-4419: a summary with "
                    "file_count:0 carries empty:true PLUS empty_reason — "
                    "project_not_registered (source exists but not under src/ "
                    "or tests/, and no .ants/project.json says where; run "
                    "project_settings op:detect then op:init), "
                    "declared_roots_hold_no_source, or no_indexable_source — "
                    "with an empty_hint and an empty_detail carrying the "
                    "measured per-directory counts. READ IT before concluding "
                    "there is no code: on the first two reasons the index is "
                    "INAPPLICABLE, not authoritative, and a session that read "
                    "the bare empty:true as \"no code here\" fell back to grep "
                    "on a live 1400-file tree. ETag-304: an unchanged query "
                ...
                docInt["description"] = QStringLiteral(
                    ...
                    "root-escaping path refuses bad_path. THREE things are "
                    "deliberately NOT reported, each counted so a suppression "
                    "cannot be mistaken for a clean pass. ANTS-4641 — a numbering "
                    "gap the document itself accounts for, i.e. a Retired / "
                    "Removed / Withdrawn section listing the missing numbers as "
                    "`- **N.** …` items: such a document usually states that the "
                    "numbers are NOT reused, so renumbering to close the gap is "
                    "the one action it forbids. The numbers land in "
                    "`heading_sequence_suppressed` as {doc: [n, …]}. ANTS-4642 — "
                    "a link whose enclosing span is a verbatim QUOTATION of "
                    "another document (a double-quoted run of 30+ characters, "
                    "which may WRAP across lines, bounded by a blank line or a "
                    "fence): the target was correct where it was written and "
                    "rewriting it would falsify the quotation. Counted in "
                    "`broken_link_suppressed`; dead_anchor loses those links too, "
                    "since an anchor inside someone else's quoted sentence is no "
                    "more ours to fix than the path is. ANTS-4643 — a "
                    "home-relative link target, which names a location outside "
                    "the project exactly as a leading slash does and is skipped "
                    "the same way, rather than reported as a file that does not "
                    "exist. Emits docs_digest — a "
                ...
                specLint["description"] = QStringLiteral(
                    ...
                    "INV-1..7 or INV-1/2 (test_coverage_unverifiable) \u2014 both ANTS-4623, "
                    "both CANDIDATES, and both skipped entirely when the Tests section "
                    "names no id at all, since comparing against nothing condemns every "
                    "invariant; `test_coverage_checked` says which happened. And a test clause that is a "
                    ...
                    "test_surface_unwired. ANTS-4666: invariant_no_test is NOT in that "
                    "list and never was gated — it fires for any INV-N with no *Test:* "
                    "clause, so an envelope naming it as skipped while returning its "
                    "findings could be read neither way. ANTS-4679: what turns the "
                    "surface checks on is a `tests/features/<name>/` directory, the only "
                    "shape this verb resolves. "
                    "ANTS-4889: a skip hint is emitted ONLY when its check was "
                    "skipped, so requesting one by name in fields= and seeing it "
                    "return in fields_unmatched is the signal the check RAN - "
                    "absence of the hint is the pass. Stated because a caller "
                    "asking for it defensively cannot otherwise tell \"check ran\" "
                    "from \"field misspelled\". "
                    "ANTS-4894: invariants_found reports how many invariants were "
                    "RECOGNISED, on every run, because ZERO is the alarm - a "
                    "document whose invariants are in a form the format standard "
                    "does not define parses to none, and every per-invariant check "
                    "then runs over an empty set while the envelope stays "
                    "comparable with one that checked them all. "
                    "ANTS-4737: counts and findings_total are taken over the FULL scan "
                    "and max_findings caps only findings[], so two runs at different caps "
                    "are comparable \u2014 the cap used to choose the count (81 uncapped, 39 at "
                    "max_findings:40 over one unchanged corpus), which silently voids a "
                    "before/after measurement. ANTS-4738: writing the marker as "
                    "<!-- required-sections: prefix --> matches a numbered entry on its "
                    "`## N. Name` prefix, so a descriptive suffix passes; verbatim stays "
                    "the default, since loosening it silently would weaken every corpus "
                    "relying on the exact match. ANTS-4739: a document carrying "
                    "<!-- spec-lint: no-required-sections --> outside fenced code is "
                    "exempt from missing_section, for a mixed corpus holding pre-standard "
                    "specs, build plans and ledgers that can never conform; the number of "
                    "exempted documents is echoed as sections_exempt_docs so an exemption "
                ...
                docCit["description"] = QStringLiteral(
                    ...
                    "SILENT. ANTS-4748 widened the grammar to a backticked "
                    "`path::symbol` (see forms_recognised, on every reply): it "
                    "resolves ONLY where the path resolves and the symbol is "
                    "unique in that file, and such a citation carries "
                    "`resolved_via:\"symbol\"` because its `start_line` was "
                    "looked up rather than authored. Anything else — an "
                    "overload, a name that has moved, an unresolvable path — "
                    "resolves to nothing and is counted as unchecked, since a "
                    "false ok is worse than an admitted silence. The "
                    "spec-format standard forbids authors writing `path:line` "
                    "— cite the symbol, not the line — so a corpus using "
                    "neither form still scans to zero with an EMPTY unparsed[] "
                    "reinforcing it: nothing was rejected because nothing was "
                    "recognised as a candidate. A zero over a document holding "
                    "citation-shaped spans (`Class::method()`, a bare "
                    "backticked path, an unresolvable `path::symbol`) carries "
                    ...
                    "hand.");
                {
                    ...
                    {   // ANTS-4386 — quotation check.
                        QJsonObject q; q["type"] = "boolean";
                            ...
                            "attributed to it by a backticked path nearby. "
                            "Emits `quotes[]` "
                            "({line, text, target, status, candidates}) with "
                            "status ok | not_found | ambiguous | no_target | "
                            "target_unresolved, plus `quote_counts` and "
                            "`quotes_checked` — a "
                            ...
                            "attribution naming several documents is "
                            "`ambiguous` WITH the hit list, never a "
                            "guess. A quotation inside a fence is a specimen "
                            "and is skipped. ANTS-4637 — so is one sitting in "
                            "a LOOP-LOG row (first cell a loop number, second "
                            "an ISO date): such a row quotes the wording a fix "
                            "REPLACED, so not_found is correct and permanent "
                            "there, and 271 of 520 unfenced quotations in one "
                            "measured corpus were of that class. The skip is "
                            "REPORTED as `quote_counts.skipped`, which sits "
                            "outside the status partition and outside "
                            "`quotes_checked` — `quote_counts_overlay_keys` "
                            "names it, so assert sum(quote_counts[k] for k not "
                            "in that list) == quotes_checked. ANTS-4638 — "
                            "attribution is PARAGRAPH-scoped: same line first, "
                            "then the nearest preceding line of the same "
                            "blank-line-delimited paragraph, because a "
                            "hard-wrapped sentence puts the path and its "
                            "quotation on adjacent lines. ANTS-4640 — inside a "
                            "TABLE the scope is the ROW: the quotation's own "
                            "cell, else the row's FIRST cell, which is its "
                            "subject by convention. An impact table names the "
                            "source first and the affected module second, so "
                            "last-path-wins picked the wrong one by "
                            "construction. ANTS-4639 — a token becomes a "
                            "`target` only if it names a document (.md, "
                            ".markdown, .txt, .rst, .adoc, .org), so a "
                            "backticked config key like `core.hooksPath` no "
                            "longer does; a bare basename resolves against the "
                            "scanned document's OWN directory before the repo "
                            "root and the basename index; and an attribution "
                ...
                psTool["description"] = QStringLiteral(
                    ...
                    "paths whenever those resolve. ANTS-4815 — the recognised "
                    "keys with no declaration are split by whether you can ACT "
                    "on them: `undeclared[]` is a to-do list (a candidate path "
                    "is on disk, so op:set will accept it), while "
                    "`unavailable[]` is not (nothing to point at, and op:set "
                    "refuses bad_path for a path that is not there). Before "
                    "the split, a project that legitimately keeps no specs had "
                    "a permanently non-empty `undeclared[]` it could only "
                    "clear by creating directories it did not want. A reason of "
                    ...
                    "ANTS-4648 — `counts_computed` says whether the walk ran "
                    "at all: on present:true it short-circuits before walking, "
                    "so `default_source_count` / `total_source_count` are "
                    "OMITTED rather than reported as 0, which read as a "
                    "measurement (a ~3000-line project reported as holding no "
                    "source). Absent = not taken; read `declared`, "
                    "`declared_missing[]` and `undeclared[]` for that path. "
                ...
                // a *_Ants_MCP_Feedback.md file instead of a full Read.
                {
                    ...
                    t["description"] = QStringLiteral(
                        ...
                        "stale binary before re-reporting — ANTS-4741 now does "
                        "that comparison for you: a shipped entry whose date is "
                        "not strictly older than this server binary's build "
                        "date carries `possibly_stale_binary:true`, and the "
                        "envelope then also echoes `server_build_date`, "
                        "`server_build_commit` and a hint, so both operands sit "
                        "in one reply. A SAME-DAY ship sets the flag, because "
                        "`shipped_date` has no time component to compare "
                        "against the build time — and same-day is the case most "
                        "likely to be misread. `format_version` "
                        "is the detected marker version (0/1/2/…). ANTS-4702 "
                        "— a version ABOVE the highest this build defines "
                        "also sets `format_version_unrecognised:true` plus a "
                        "`format_version_hint`. The delta rule is \"v2 or "
                        "higher\", so such a file parses and every verb works; "
                        "the marker is still wrong, and without the flag the "
                        "widening is invisible until some verb gates on an "
                        "exact version and fails far from the file that caused "
                        "it. The delta is sound either way — this reports the "
                ...
                // structured args (no hand-edited markdown).
                {
                    ...
                    t["description"] = QStringLiteral(
                        ...
                        "ANTS-4613: a leaf beginning with a DOT (~/.claude) "
                        "refuses `bad_args` with `candidates` instead of "
                        "deriving — such a name can never match the "
                        "*_Ants_MCP_Feedback.md glob, so the file it created "
                        "was one nobody would ever read. Name it explicitly. "
                ...
                // edit break?" before the edit, not after.
                {
                    ...
                    t["description"] = QStringLiteral(
                        ...
                        "alone. Matching is substring-only (no symbol "
                        "resolution). ANTS-4644 — pass the project-relative "
                        "path (`src/foo.cpp`); if that matches NOTHING and "
                        "specs were scanned, the verb retries with the path's "
                        "suffixes, longest first, because a spec cites a "
                        "module the way its author writes it. `fallback_match` "
                        "is on EVERY reply; when true, `fallback_kind` is "
                        "\"path_suffix\" or \"basename\" and `matched_as` maps "
                        "each path you passed to the form that actually "
                        "matched — check it, since the basename tier (used "
                        "only when every fuller form failed) can collide "
                        "between two like-named modules. ANTS-4645 — "
                        "`roadmap_scanned` is always false: this reads "
                        "docs/specs + docs/phases and NEVER the ROADMAP, so "
                        "work already planned there does not appear here even "
                        "on a confident multi-spec answer — use roadmap_query "
                        "(query=<keyword>) or task_priors for that. Use "
                    ...
                    filesProp["description"] = QStringLiteral(
                        ...
                        "false hits on basename collisions. ANTS-4644 — a "
                        ...
                        "ANTS-4742: a zero that is NOT `scanned_nothing` now "
                        "sets `path_match_only:true` with a hint. Every tier "
                        "matches the PATH, so a spec citing the module by "
                        "SYMBOL carries nothing this can match and the zero "
                        "does NOT establish that no spec governs the file — "
                        "which is the dangerous direction, since write-code's "
                        "Phase 0 makes this the first lookup and a false zero "
                        "sends the session to write against no contract. The "
                        "hint names the fallback: search the module's stem "
                        "across the specs dir with workspace_search. The "
                        "matching is deliberately unchanged — folding a bare "
                        "stem into `matched_specs` would hide a weaker match "
                ...
                // context. Engine: src/indiereviewengine.{h,cpp}.
                {
                    ...
                    t["description"] = QStringLiteral(
                        ...
                        "ANTS-4804 — every lane also carries `total_lines`, "
                        "and a lane over the line budget is `oversized:true` "
                        "with envelope `oversized` / `oversized_lanes` / "
                        "`oversized_hint`. Read BOTH: `too_coarse` counts "
                        "files and is silent on a lane that is one large "
                        "module, which is the shape most likely to be swept "
                        "as a single unit. `total_lines` is reported whether "
                        "or not the threshold trips, so apply your own budget "
                        "if it differs; `total_lines_capped` means the figure "
                        "is a FLOOR, not a total. "
                        "ANTS-4806 — a lane over a declared or detected "
                        "test root carries `kind`:\"tests\". review-code's "
                        "scope excludes tests and review-tests' is exactly "
                        "them, so DROP what you do not want rather than "
                        "expecting the verb to choose: it labels and never "
                        "omits, because a silently omitted tree is the failure "
                        "ANTS-4771 was filed about. An absent `kind` is the "
                        "absence of a label, not a claim the lane is "
                        "production code. ANTS-4805 — .github is walked as "
                        "an explicit root, so the CI and release surface "
                        "appears in a lane or in the coverage fields below "
                        "rather than vanishing. ANTS-4816 — a lane whose "
                        "files the count cannot admit (packaging, YAML, "
                        "shell) carries `uncounted_files` and "
                        "`counted_nothing`, so a `file_count` of 0 is "
                        "distinguishable from an empty directory; both "
                        "`too_coarse` and `total_lines` key on that count, so "
                        "neither fires on such a lane. "                        "ANTS-4771/4786 — files under the walked source roots "
                        "that landed in NO lane are reported as "
                        "`unassigned_count` / `unassigned_by_suffix` / "
                        "`unassigned_reason` / `unassigned_hint`, on BOTH the "
                        "declared and the computed path, plus a bounded "
                        "`unassigned_sample` of paths on the declared one. "
                        "`unassigned_reason` names the cause: "
                        "`suffix_filter` (a `derived:true` reply — the "
                        "computed walk admits only suffixes the codebase "
                        "index can outline, which is NARROWER than "
                        "\"source\", and shell in particular is dropped) or "
                        "`incomplete_partition` (the project's declared "
                        "partition does not name them). Read either as a "
                        "coverage gap rather than a formatting note — a "
                        "review driven by this partition will not look at "
                ...
                // docs/standards/roadmap-format.md § 3.5.
                {
                    ...
                    t["detail"] = QStringLiteral(
                        ...
                        "§ 3.10.2. ANTS-4606 — this verb SPELLS that "
                        "dialect `gfm`, where roadmap_query spells the "
                        "same one `github-task-list`. One format, two "
                        "vocabularies, so a caller comparing the two "
                        "envelopes reads a disagreement that is not one. "
                        "Both stand deliberately: unifying them would "
                        "silently break any caller already keying on "
                        "either, and the store is machine-global, so that "
                        "caller may be another project's session. Map both "
                        "spellings; never compare by string equality "
                        ...
                        "is not the same as clean. ANTS-4729 — the BOOL asks "
                        "whether the publish overwrote content the FILE held, "
                        "so it is computed from the file's own classified "
                        "lines. A line the RENDER adds and the file lacked — "
                        "the format marker, on a project rendered before that "
                        "marker existed — is an addition, never a discard, and "
                        "no longer fires it. `discarded_edit_lines` keeps its "
                        "two-directional count. It reports and never "
                        ...
                        "them. ANTS-4615 — the total alone could not be acted "
                        "on: one measured report of 84 was 24 bullets restyled "
                        "into the canonical id form plus ONE sentence that no "
                        "longer existed anywhere, and a number mixing those "
                        "trains you to wave the flag through. The true arm now "
                        "also carries `discarded_restyled_lines` (text "
                        "survives, styling differs) and "
                        "`discarded_text_lines` (text does NOT survive — THIS "
                        "is the one to act on), plus `discarded_text[]` naming "
                        "the lost lines, capped at 20 with "
                        "`discarded_text_truncated`. ANTS-4695 — a third, "
                        "`discarded_repunctuated_lines`, rides the same arm "
                        "when present: a file line differing from the render "
                        "ONLY in terminal punctuation. It is neither of the "
                        "other two, and it used to score as restyling because "
                        "the content key ignores punctuation entirely — so a "
                        "project whose Layman lines were hand-authored with "
                        "periods saw all of them change while "
                        "`discarded_text_lines` read 0, which is the field a "
                        "caller checks before allowing the overwrite. The "
                        "counters classify "
                        "the FILE's lines and so do not sum to the total, "
                        "which also counts lines the render restores that the "
                    ...
                    opProp["description"] = QStringLiteral(
                        ...
                        "\"set_body\" (ANTS-4808) REPLACES a bullet's WHOLE "
                        "body — locate by id/anchor/headline and pass "
                        "`new_text` alone. `old_text` is REFUSED "
                        "(bad_op_combo) because it scopes nothing, and an "
                        "empty `new_text` clears the body. It is the route "
                        "back for a body whose text cannot be expressed as a "
                        "match, which amend_body has no answer for: three "
                        "sessions reported ending there, one leaving a "
                        "kilobyte of garbled prose in a shipped item. "
                        "Store-backed projects only. It re-derives the "
                        "trailer columns and records the previous body in "
                        "history exactly as amend_body does, and the envelope "
                        "echoes `replaced_body_chars` so the reply says what "
                        "was destroyed. dry_run previewable. "
                    ...
                    oldTextProp["description"] = QStringLiteral(
                        ...
                        "ANTS-4612 — the wrapped pass DECLINES a span whose "
                        "continuation lines carry structure a re-flow would "
                        "destroy: column alignment (an internal run of 2+ "
                        "spaces) or the opening of a new list item. Refuses "
                        "`body_match_wrapped_block`, nothing is written, and "
                        "the remedy is to amend ONE line of the block or "
                        "rewrite it so it does not depend on alignment. A "
                        "distinct code because `wrapped_match:true` is also "
                        "true on the benign case, so it cannot be branched on. "
                        "Differing INDENTATION is not the trigger — a bullet's "
                        "hanging indent differs and is an ordinary hard wrap. "
                        ...
                        "works on BOTH backends, by different routes. On a "
                        "MARKDOWN project it patches the bullet line and "
                        ...
                        "replaces. ANTS-4668 — on a STORE-BACKED project it "
                        "writes the `headline` COLUMN and re-renders, the same "
                        "path op:\"flip\" and op:\"annotate\" already take. "
                        "There is no prefix guard on that route because the "
                        "column holds the headline TEXT alone (the marker, id "
                        "brackets, emoji and bold delimiters are composed by "
                        "the render); the guard there is bad_args on a "
                        "`new_text` that would leave the headline EMPTY, which "
                        "no locate could find again. It formerly refused "
                        "unsupported_format on a store-backed project, leaving "
                        "a headline immutable for the life of the item while "
                        "body notes accumulated a correction the headline went "
                        "on contradicting. Refusals are "
                        "headline_match_not_found / headline_match_ambiguous; "
                        "the store route echoes the new `headline`. "
                    ...
                    dryRunProp["description"] = QStringLiteral(
                        ...
                        "would-be id under `would_be_id` / `would_be_ids` "
                        "(ANTS-4508 — NOT `id`/`ids`, which a caller reading "
                        "one field takes for a reservation; nothing is "
                        "reserved and the id is wrong if any other write "
                        "intervenes), formatted bullet(s) and 1-based "
                ...
                // whole-project write verbs.
                {
                    ...
                    t["description"] = QStringLiteral(
                        ...
                        "instinct, and it left a permanent row that "
                        "roadmap_query mode:\"report\" scope:\"all\" sums into "
                        "machine-wide figures forever. Keyed by caller_cwd's "
                        "root, or by `export_slug` when the files are already "
                        "gone. It REFUSES `confirm_required` while the root "
                        "still exists on disk, because the store is primary and "
                        "ROADMAP.md is its render — the rows are the only copy "
                        "of the history, relationships and citations the file "
                        "does not carry. Pass `confirm:true` to override; an "
                        "absent root needs no ceremony. `dry_run:true` reports "
                        "what would go WITHOUT deleting it. The delete is one "
                        "transaction across all nine tables in foreign-key "
                        "order and rolls back whole on any failure. Distinct "
                        "from ANTS-4600's `transient_root` guard, which stops a "
                        "scratchpad under the system temp dir being registered "
                        "at all. "
                        ...
                        "source:\"markdown\". Both facts are now FIELDS a caller "
                        "can branch on rather than prose it must read: "
                        "`store_backed` (will roadmap_query serve this project "
                        "from the store afterwards — the source-dialect answer, "
                        "unchanged by dry_run) and `markdown_rewritten` (always "
                        "false). ANTS-4479 — `updated_items:[{id,fields}]` names "
                        "WHICH items `items_updated` counted and which columns "
                        "moved, so a dry run is a reviewable plan rather than a "
                        "bare count; capped at 200 with `updated_items_truncated`, "
                        "and `items_updated` stays the true total. ANTS-4490 — "
                        "`sections_unchanged` accompanies `sections_written`, so "
                        "`0 written` reads as an idempotent re-run rather than as "
                        "a dead counter. ANTS-4694 — `elements_written` counts "
                        "the NON-ITEM content carried: prose, older-dialect "
                        "bullets, tables and legends all become `narration` / "
                        "`table` elements under the section they sat in. The "
                        "item count is therefore NOT a coverage figure, and on "
                        "a prose-heavy roadmap it is small by construction — a "
                        "`coverage_hint` says so on the arm where that could "
                        "mislead (more elements than items). Nothing is "
                        "dropped: the line partition is total and tested as "
                        "one, so every non-blank source line lands in exactly "
                        "one item, element, section or legend. ANTS-4478 — "
                        "`project_id` is the real id "
                ...
                // ANTS-1548 — changelog_log: token-frugal CHANGELOG writer.
                {
                    ...
                    clOp["description"] = QStringLiteral(
                        ...
                        "`date` defaults to today and the RESOLVED value is "
                        "echoed back as `date`, on the dry run as well as the "
                        "write — ANTS-4833, because it is the release path's "
                        "only unpredictable field and the heading date is what "
                        "a notes-extraction grep keys on), returns the closed "
