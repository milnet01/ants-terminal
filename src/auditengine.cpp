...
QString computeDedup(const QString &file, int line,
    ...
    // ANTS-4452 — multi-arg .arg() substitutes every placeholder in ONE
    // pass. A chained single-arg call does not: each call re-scans text an
    // earlier one already wrote, so a literal "%3"/"%4" inside `file`
    // (legal in filesystems) is read as a placeholder and two distinct
    // findings collide onto one dedupKey. Qt documents the multi-arg
    // overload as existing for exactly this. The comment here previously
    // claimed the opposite and recommended the unsafe form.
    const QString raw = QString("%1:%2:%3:%4")
                           .arg(file, QString::number(line), checkId, title);
