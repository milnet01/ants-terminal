...
- **Status is derived, never stored.** A reader resolves each assigned id's
  ...
  ANTS-3504 (spec-approved, pending); see §"Tooling" and §"Stale-binary
  self-check".) The
...
- **Compaction is roadmap-driven.** Once an assigned id is ✅ in the roadmap,
  ...
  (ANTS-3504, spec-approved/pending), lifted from the id's `Resolved` roadmap
  line (parens optional), so a contributor on an old binary can compare it
  against their `session_orient`
...
The `2026-07-12` is the fix's **ship-date** (ANTS-3504) — the **last** `Resolved`
date (parentheses optional) in the id's `ROADMAP.md` bullet body; a finding
mapped to several shipped ids stamps the **latest** of their ship-dates, and a
bullet with no parseable `Resolved` date keeps the pre-3504 dateless form
(`→ shipped ✅ (write-up compacted, ANTS-3443)`). See §"Stale-binary self-check"
for the exact extraction rule, the date-not-version/commit rationale, and the
contributor self-check convention.
...
> **Status: spec-approved, not yet implemented** (`docs/specs/ANTS-3504.md`). The
> dated stub and `feedback_query` `shipped_date` described here are the *target*
> contract. Until it ships, `compact_resolved` emits the dateless stub and
> `mapped_id_status` carries `{id, status}` only.
...
| `feedback_log op:compact_resolved` **(shipped, ANTS-3443)** | Auto-collapse shipped findings' write-ups; gates on live roadmap ✅. Refuses `not_v2` on a v1 file. (A `drop_prose` option is **deferred** — see §"Maintainer compaction"; NOT in ANTS-3443 scope.) **ANTS-3504 (spec-approved, pending)** stamps the fix's ship-date into the stub (`→ shipped ✅ <date> (write-up compacted, ANTS-3443)`; latest `Resolved` date among the finding's ✅ ids, dateless fallback when none) — §"Stale-binary self-check". |
