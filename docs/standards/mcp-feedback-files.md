...
- **Compaction is roadmap-driven.** Once an assigned id is ✅ in the roadmap,
  ...
  the canonical form; `compact_resolved`). ANTS-3504 (pending) will add the fix's
  ship-date to that breadcrumb (`→ shipped ✅ <date> …`), lifted from the id's
  `Resolved` roadmap line (parens optional), so a contributor on an old binary
  can compare it against their `session_orient` `server_build.build_date` and
  relaunch rather than re-report an already-shipped fix (§"Stale-binary
  self-check"). Nothing goes stale because nothing is stored
  to go stale.
...
finding body collapses to a `→ shipped ✅ (write-up compacted, ANTS-3443)`
stub that retains the `**Proposed ID:**` line (ANTS-3504, pending, will add the
fix's ship-date to that breadcrumb; the canonical form is in §"Maintainer
compaction"). Status is never
