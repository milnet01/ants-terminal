...
| `docs_index` | ANTS-2139 | Disk: `~/.cache/ants-terminal/docs-index/<sha256(canonical root)>.json` | `sha256(canonical resolved root)` (one file per project) | **Orphan.** Same profile as `codebase_index`: relocated root → new SHA → fresh build; version- or root-mismatched body discarded + rebuilt (never shadows). Purely derived — safe to delete. |
