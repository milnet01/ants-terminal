...
one**, rather than restating the reasoning (§2.1). **Scope: a review
instrument whose findings gate a document** — the `review-` and `check-`
skills. It does not reach a human code review, a PR checklist, or a
project's own process. A document that genuinely needs prose work gets a
separate, single-pass, report-only job. Never a gate.
