...
**Everything above is an authoring obligation, never a review question**
...
sees two copies only when they disagree. `draft/skills/README.md` owns the
...
| §2.3 no census counts | `check-doc-facts` `counts`, a census-count check, which names §2.3 as its own authority. **`Partial:`** the `pre-commit` hook has no count check of its own (§9.0), so a census reaches a commit uncaught. Added 2026-08-20; §2.3's larger half had no row while its `path:line` half did |
| §2.2 cited symbols exist | `check-doc-facts` `symbols`, a symbol-resolution check against current source |
| §2.9 required sections present | `check-doc-facts` `sections`, a required-section check against the format standard, and `mandated-section`, the same check for a section the document's class requires. **`Partial:`** both test PRESENCE; **nothing** checks that a mandated section is in the mandated PLACE, though §2.9 requires `## What checks this` last and the loop log after it (added 2026-08-20, when §9.4 credited a position test that does not exist). This row named `structure` until 2026-08-17; that check is fences, id gaps and parse |
