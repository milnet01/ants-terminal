...
| [versioning-overrides.md](versioning-overrides.md) | The two answers the global versioning standard leaves to each project — its § 3 breaking surfaces, asked of every project, and its § 4 one-line `1.0` exit condition, asked of a `0.x` one. Also carries this project's one override: inside `0.x` the MINOR is the milestone rather than a fact about breakage, so read it before choosing a release's level. |
