...
| `needs_triage` | A bulk defer of un-triaged FP-prone findings was refused. Envelope carries `total`, `non_auto_fixable`, `threshold`. More than `threshold` (25) non-auto-fixable (judgment-required) findings can't be folded into ROADMAP without `triaged:true` — the guard against dumping raw scan output as real items (ANTS-3346). | `debt_sweep_defer` with a large un-reviewed non-auto-fixable batch. |
