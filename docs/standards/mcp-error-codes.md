...
| `bullet_not_found` | A `roadmap_log` flip/annotate locator (id / anchor / headline) matched no bullet. Distinct from `format_mismatch`, which fires when the roadmap is a recognised-but-unwritable format (e.g. `#### Pass N.M` headings, ANTS-2031). | `roadmap_log op:"flip" id:"ANTS-9999"` when no such bullet exists. |
...
| `format_mismatch` | The file is a *recognised* format the verb can read but can't *write* (so `unrecognised_format`, which keys on zero parsed bullets, doesn't apply). Envelope carries `format`, `path` (the recognised file), and an Edit-fallback `hint` (ANTS-2031 / ANTS-2040). The writer/reader format-parity rule (mcp-tools.md §6a, ANTS-2042) mandates this code over a generic absence code whenever discovery recognises a format the writer can't produce. | `roadmap_log` append/flip/annotate against a `#### Pass N.M` heading roadmap (`format:"pass-headings"`); `changelog_log` against a `data/changelog.yaml` (`format:"yaml"`, ANTS-2040) — readable, but no writer for that format yet. |
