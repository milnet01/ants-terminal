...
| `not_v2` | `feedback_log op:"compact_resolved"` (ANTS-3443) against a feedback file whose version marker is not `<!-- ants-mcp-feedback: 2 -->` (a `: 1` marker, or malformed/absent). The op is v2-only — a v1 file's findings predate the structural `**Proposed ID:**` line, so it would be a silent no-op; refuse and direct the caller to `op:migrate_v2` first. | `feedback_log op:"compact_resolved"` on a `<!-- ants-mcp-feedback: 1 -->` file. |
...
| `module_map_unparseable` | `indie_review_orchestrate`: a `## Module map` heading IS present but no reviewable lanes could be derived — its bullets aren't the `- <subsystem-name> — <summary>` shape the parser reads (e.g. a `- <path> — <description>` file list), or the named subsystems resolve to no source files (ANTS-3481). Distinct from `no_lanes` (heading genuinely absent) so a session doesn't wrongly conclude it has no map. Envelope names the resolved source file + the expected bullet shape. | `indie_review_orchestrate` on finbreak's CLAUDE.md, whose `## Module map` is a `- path — description` list. |
