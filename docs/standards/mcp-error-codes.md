...
| `no_roadmap` | `caller_cwd` doesn't canonicalise to a directory, or no ROADMAP.md was found under the resolved root. | `roadmap_log caller_cwd:"/no/such/path"`. Used by every `roadmap_log` op (ANTS-1424, 1878, 1879), and by `test_audit_fold_in` / `test_audit_recheck`. Semantic twin of `no_changelog` in § 2 — the two are categorised differently for historical reasons only. |
