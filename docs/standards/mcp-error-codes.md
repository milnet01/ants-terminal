...
| `body_shadowed` | A store-backed `roadmap_log` write supplies a trailer **column** (`kind` / `layman` / `source` / `lanes` / `evidence`) that the `body` in the same request would hide (ANTS-3809 § 2.5). A rendered bullet is head line, then body, then the canonical trailer lines, and every matcher takes its **first** match — so the body's occurrence is reached before the column's own line, and the bullet would render correctly while re-parsing wrongly. Fires on **value difference** only (a body agreeing with its column is the ordinary case and does not refuse), over all five keys regardless of whether the body's match begins a line. Reaches only `append` / `append_batch` — every other op re-derives its columns from the body it just wrote (§ 2.6), so its column cannot disagree. Per **bullet** on `append_batch`, landing in `skipped[]`. The message quotes the shadowing text and names the remedy: delete the stale trailer line, or reword the mid-prose mention. | `roadmap_log op:"append" kind:"fix"` with a body carrying a stale `Kind: refactor` continuation line. |
...
| `unsupported_format` | The verb *recognises* the file's format and other ops of the same verb write it, but **this op** has no writer for it — narrower than `format_mismatch`, which says the whole verb cannot write the format. The distinction is per-op, and it is why `roadmap_log` emits both codes on the same file: on a `#### Pass N.M` roadmap `create_section` refuses `format_mismatch` (no heading-format section writer at all), while `amend_body` refuses `unsupported_format` (a pass body is sub-bullets under a heading, not the indented continuation lines it patches) — five other ops write that format happily (ANTS-2126 / ANTS-3406). Envelope carries the Edit-fallback `hint`; unlike `format_mismatch` it does not promise `format` / `path`. | `roadmap_log op:"amend_body"` against a pass-headings roadmap. |
