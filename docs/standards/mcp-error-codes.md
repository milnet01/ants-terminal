...
| `bad_cwd` | `caller_cwd` is present but does not canonicalise to an existing **directory**. Distinct from `caller_cwd_required` (wholly absent) and from `bad_path` (a *path argument*, not the cwd). Emitted at the dispatch lambda, before the verb's engine runs — so for the verbs that use it (`audit_run`, `audit_poll`) this, not `bad_path`, is what a caller sees for a bad cwd. | `audit_run caller_cwd:"/no/such/dir"` (ANTS-1833). |
...
| `bad_severity_floor` | A severity/level-floor argument isn't one of the accepted levels. | `last_audit_summary level_floor:"catastrophic"`. |
...
| `stale_partition` | A `partition_token` echoed back by the caller no longer matches a recompute — the file set changed under it, so any chunk ids derived from it are unsafe. Checked before any cache lookup. | `test_audit_brief` with a token from before a file was added (ANTS-1397 INV-4). |
