...
**The four numeric columns were `CRIT | HIGH | MED | LOW` until loop 7, and
rows 4–6 are read that way.** The header migrated to the four questions on
2026-08-17, as every sibling standard's had already done; the legacy rows are
untouched, per rule 14's ban on rewriting them.
...
| 7 | 2026-08-17 | 3, cold — genre pinned `standard`. First gate since 2026-07-27 and the first ever under the four questions. Packet carried the live `spec_lint` / `spec_query` / `spec_log` schemas, `documentation.md` § 2.9 and the `what-checks-this` catalogue entry in full, and named the out-of-repo paths as an unrunnable region | 4 | 5 | 0 | n/a | **Nine verified, nine fixed, none dismissed. Six of the nine were found by two or three lanes independently, and the two best were settled by running the tool rather than by reading it.** **All three lanes found the same two.** § 6 said `spec_log op:append_loop` writes bullet form and sent the author to `Edit` against a table — ANTS-4364 made the verb match the section's shape and *refuse* rather than corrupt, so this standard steered every spec author away from the one supported path. And § 6's *"looped until it converges"* stood against § 5.4's hard cap, giving one gate two stopping conditions; § 5.4's own *"split it, not keep looping"* was the same contradiction from the other side, against the skill's *"a spec reaching the cap is normal — ship it"*. **Neither empirical finding came from a lane's reading.** Resolving a lane open question by linting a six-form fixture showed § 3.7's prescribed tombstone — `*withdrawn 1.4.0: …*`, added 2026-08-14 by CFG-0108 — comes back as `invariant_no_test` while `*withdrawn — 1.4.0: …*` is exempt, so every correctly withdrawn invariant drew a false finding in the bucket callers act on without re-verifying. The same fixture returned `sections_source: "standards/spec-format.md"`, proving § 3's lookup list omits the un-prefixed tier that a repository which *is* a standards set actually resolves to — it had named the `docs/` pair alone. **§ 5.8 described a `what-checks-this` sibling removed on 2026-08-11** for flagging nearly every live cell in `coding.md` and `commits.md` — which is exactly why this file's own three `size` citations survived a week unfired-on. **Two contradictions were this machine's own fresh collateral:** § 3.12's *"two spec-side specifics"* against `documentation.md` § 2.9's *"it states no rule of its own"*, written the day before, and § 0's `nothing`-row **count** against the footer's **share**, where the row-split the footer requires raises the count and moves nothing. **Fixed next door, five sentences and none of them this document's:** `documentation.md` § 2.9 twice, its § 9.0 claim that the `what-checks-this` check resolves *every* name a cell gives, `spec-skeleton.md`'s Layman comment telling an author to delete the line for internal work where § 5.6 requires it always, and `run-mechanics.md`'s own copy of the `append_loop` claim. **Three lane open questions resolved clean and are not in the tally:** `contract` does cover the breaking-input clause, the checks named in four table rows all exist, and § 3.2's *"(§6)"* is not false — this standard's § 6 does own the loop-log rules. |
| 8 | 2026-08-17 | 3, cold — identical brief, scrubbed copy rebuilt from disk | 3 | 2 | 1 | n/a | **Six findings: four verified and fixed, two dismissed. The run's largest lesson is a defect in the packet, not in the document.** **All three lanes filed the same finding and all three were wrong**, because the packet still quoted `documentation.md` § 2.9's *"it states no rule of its own"* — which loop 7 had replaced hours earlier with the sentence § 3.12 now agrees with. The two documents had been reconciled; the brief said otherwise, and a lane cannot outrank its own packet. Third occurrence of this class here (ROADMAP CFG-0134), and the first where the stale quote was this run's own fix: rebuilding the scrubbed copy and the changed facts is not rebuilding the packet. **Two lanes found the Q1 with the widest reach.** § 7 listed the invariant bullet form among the things needed *"to stay parseable"*, while `spec_query` reads the GFM table form too and § 3.7 says so outright — so an author holding a conforming table was told to reflow a list § 3.7 forbids reflowing, churning `INV-N` handles cited from CHANGELOG and sibling specs. **One lane found a check declared unbuilt that has existed since 2026-08-10**: the § 3 block-vs-headings row read *"a candidate check for `check-doc-facts`; not built"*, and `enumeration-parity` is that check. The cell is now `Partial:` — it takes the in-document pair, and the block-against-skeleton pair is cross-document and therefore `review-contract-set`'s. **All three lanes found § 5.6 carrying no row at all** — a rule this gate had made *more* prescriptive one loop earlier; § 5.5 had the same hole and is fixed with it. **One finding came from verification rather than a lane:** the id-gap row named `structure` with no bucket, where the catalogue makes a gap a CANDIDATE precisely because only the author knows whether an id was withdrawn — and a hard-finding reading invites the renumbering § 3.7 forbids. **Dismissed, both Q2:** the § 3.12 packet defect above, and a proposal to split the id-gap row on the ground that `structure` needs the MCP — its id-gap half does not, and this table already marks the parser-dependent row *(via `spec_query`)*. **Fixed next door:** `write-spec`'s own copy of the `append_loop` claim, the third found in three days. **Three lane open questions resolved clean:** the skeleton does ship a table log, `sections` does exist, and `structure` does carry the id-gap check. The honest error budget moved from 7 of 17 rows to 8 of 19 — **up**, because two uncovered rules stopped being invisible. |
| 9 | 2026-08-17 | 3, cold — identical brief; the WHOLE packet rebuilt from disk this time, every citation and path re-verified, and the skeleton's loop-log header and layman placeholder added as facts | 3 | 7 | 1 | n/a | **Eleven findings: nine verified and fixed, two dismissed. Cap reached (3 for a standard); the run files its tail and exits. Four of the nine were this gate's own collateral from loops 7 and 8** — the 4a-min pattern for the fifth consecutive gate on this machine, and again every one landed in text a fix had *added*. **Two lanes proposed opposite fixes for the same rule, and naming the owner settled it.** One held that the § What-checks-this cell added in loop 8 blurred a catcher with a `nothing` and must split; the other that the *"a row splits rather than blurs"* paragraph is itself a diverged restatement of `documentation.md` § 2.9's cell forms — which § 3.12 delegates away, and which license exactly that shape as **`Partial:`**. The owner wins: the paragraph is gone, and the cell stays. **The widest Q2 is an id shape, and running the parser settled it.** § 2 was widened on 2026-08-14 to admit `CL-9` and `PASS-3-1` and to forbid reshaping an id; § 3.1, § 3.2 and § 7 were not widened with it, and § 7 called `<PREFIX>-NNNN` a *parse* requirement. A spec headed `# CL-9 — id shape probe` parses whole — title, status, kind and invariant all returned — so an author was being told to pad an id and desynchronise the H1 from the filename § 2 fixes. Both skeletons carried the same template and are fixed with it. **§ 6 misstated the requirement it defers to**: `documentation.md` § 9.1's second is that the *tally balances*, and this said *every row carries an outcome* — weaker, and a conformer could satisfy § 6 in full while breaching § 9.1. **§ 9 told a maintainer the loop log is governed by § 6 *rather than* by the required-structure list**, while § 3's block carries `## 12` and `missing_section` reads that block verbatim; deleting the line on § 9's authority would have removed the only mechanical presence check § 6 has. **§ 4's heading says *Recommended* and its first bullet says *required*** for any feature holding state, with no row admitting nothing catches it. **The shipped budget command was case-sensitive** — `/\*\*nothing/` — and this file's own loop-8 cell wrote `**Nothing**`, so the number § 0 calls the one that matters was silently low. Fixed and verified against a capital-N probe. **Dismissed, with reasons:** *Definition of done* is not undefined — `plan-skeleton.md`, which § 8 names a paragraph above, ships the section; and the id-gap row's *candidate* bucket is right, because the row names `check-doc-facts` `structure`, whose catalogue entry buckets id gaps as CANDIDATES, not `spec_lint`, whose `invariant_id_gap` is a finding. Its *rationale* was wrong and is fixed. **Four lane open questions resolved clean**, and `roadmap-format.md` § 3.5.1 does admit an underscore prefix (`3D_E`). Honest error budget: 9 of 20 rows, up from 7 of 17 at loop 7 — three loops of this gate found three more rules nothing catches, and said so. |
<!-- MIRROR END -->
