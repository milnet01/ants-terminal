...
**An archive file gets steps 3 and 4 for the sections routed to it, and no
legend** — the legend belongs to the project, not to a file, and the migration
only ever reads one from source 0.

**It also gets no step 1, and that is why the marker is not purely a replay
(amended at implementation, 2026-08-03).** The synthetic root has an **empty
slug** under `UNIQUE (project_id, slug)`, so there is exactly **one per
project** — `RoadmapMigrateLoad` resolves it with `findSection(projectId, "")`.
An archive therefore has no root section of its own and no stored intro to
replay, so an archive rendered from stored data alone would ship with no format
marker and stop being parsed as a conforming file. The rule is therefore:
**replay the marker where a root intro carries it, emit the constant where no
root section routes to this file, and never both** — which is what INV-8's
"exactly once" tests in each direction.

A consequence worth stating rather than discovering: because there is one root
per project, **an archive's own pre-heading content has nowhere to live in the
store today**. That is a limitation of the migration rather than of the render,
and it is filed as its own roadmap item; this spec only declines to pretend the
render can recover what was never stored.
