...
`tests/features/osc133_rerun_safety` does. Expected bytes are captured from
the pre-change functions on the same fixture and pasted into the test.
...
- **INV-5** — A line that leaves scrollback before it is written fails the
  export, and no file exists at `path`. The fixture sets a small
  `setMaxScrollback`, runs one `step(1)`, then feeds lines until an unwritten
  line is dropped. Broken by removing the offset check, which reads shifted
...
- **INV-9** — No caller builds a whole export. `contextMenuEvent` and
  ...
  `exportAsText`, `exportAsHtml` or `exportBlockAsCast` remains in `src/`.
...
- **INV-10** — `startExport` runs in slices and reports. Its slice calls
  ...
  Broken by a loop that runs `step` to `Done` in one call. *Test:*
...
`SRC_TERMINALWIDGET_PATH`; it adds a path define for `src/mainwindow.cpp`.
...
INV-8, INV-9 and INV-10. Capture INV-1 to INV-3's expected
bytes from the pre-change functions before removing them. Verify each test
fails against pre-change source first. INV-1 to INV-8 fail to compile
...
- `src/terminalwidget.h` — the comment above the removed
  `exportBlockAsCast` declaration.
