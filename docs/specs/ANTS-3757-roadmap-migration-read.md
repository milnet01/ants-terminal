...
- Whenever normalisation was **not an identity**, the author's verbatim string
  ...
  constraint is untouched. An **absent** Status line is a different case and
  takes § 3.3's route instead — `planned` with `provenance.status =
  "defaulted"`, no `extras.source_status`, because there was no author string to
  preserve.

  **This use of `migrated` does not fit the definition the standard currently
  gives it, and the gap is real rather than a wording slip.**
  `roadmap-data-model.md` § 7.7 defines `migrated` as "generated by migration
  itself, **with no source-side counterpart**" — written for § 7.2's allocated
  ids, where nothing existed before. A normalised status has a counterpart: an
  author wrote `done`. But `asserted` is worse, because it means "an author
  supplied it" and would make migration's `partial` → `planned` guess
  indistinguishable from a status the author actually chose, which is the exact
  confusion § 7.7 exists to prevent. This spec takes `migrated` on the ground
  that migration *decided* the stored value, and § 7 records the amendment
  § 7.7's gloss needs. **Surfaced for decision** — it changes a published
  standard's enum semantics, not just this document.
...
- `docs/standards/roadmap-data-model.md` — three changes, and they are not
  equal:
  ...
  - § 7.7's gloss on `migrated` ("with no source-side counterpart") does not
    cover a value migration *transformed* from an author's. **Decision pending**
    — § 2.7 states this spec's position and the reasoning; the standard's author
    settles it.
