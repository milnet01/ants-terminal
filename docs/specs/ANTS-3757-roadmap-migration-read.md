...
**ANTS-3767** on 2026-07-31 (locked by ANTS-3756 INV-21); the last two —
`projectId` and `sectionId` — landed with **ANTS-3765** on 2026-08-01, both
resolved inside its transaction, which is where they had to be: a project row
written before it would claim a migration that had not happened (that spec's
INV-11).
...
*(`dropped` half superseded by ANTS-4977: a dropped item is published as 🚫.)* **`dropped` is never produced by migration, and that is a property of the source
...
**The set is closed for the READ half and is extended once, by the load half.**
[ANTS-3765](ANTS-3765-roadmap-migration-load.md) § 2.11 adds **seven** load-only
codes — `orphaned_item`, `id_allocated`, `history_capped`, `field_conflict`,
`project_refused`, `bad_options` and `ambiguous_rematch` — and they are named
here rather than only there because `Note::code` is **one type shared by both
halves**: two documents each claiming to hold the whole vocabulary is exactly
the drift this table exists to prevent. Every one of them carries `line = 0`,
which this type defines as whole-file, because a load note is about a store row
and the load never opens the source file.
...
- **`extras`, `lanes` and `evidence` had no write path at all** — a defect in
  ...
  **ANTS-3767 shipped it on 2026-07-31** — `ItemWrite` carries all three,
  ...
  check — so the blocker on ANTS-3765 was cleared. **ANTS-3765 then shipped on
  2026-08-01 and discharged the last two rows**, `projectId` and `sectionId`,
  leaving § 2.1.1 owing nothing. It is
...
- **[ANTS-3765](ANTS-3765-roadmap-migration-load.md) amends this document twice,
  and both landed on 2026-08-01 when it shipped.** § 2.1.1's last two owed rows
  are discharged above. And **§ 2.10's note-code set, which this spec calls
  closed, gains that half's seven load-only codes** — `orphaned_item`,
  `id_allocated`, `history_capped`, `field_conflict`, `project_refused`,
  `bad_options` and `ambiguous_rematch`, now listed under that table. Seven and
  not the six its own § 7 predicted: `ambiguous_rematch` arrived with § 2.6.1 at
  that spec's third cold-eyes loop and its § 7 line was not re-counted, which is
  the arithmetic drift a set stated in two documents produces and the reason
  they are listed here rather than merely referred to. `Note::code` is one type
...
| 4-impl | 2026-08-01 | none — implementation of the LOAD half, not a review | — | **Fold-back row, written by ANTS-3765's implementer.** That half shipped, and the two amendments its § 7 owed this document are applied: § 2.1.1's last two owed rows (`projectId`, `sectionId`) are discharged, so the table now owes nothing and "the load half copies rather than translates" is true of every field; and § 2.10's closed code set is extended, once, by the load's **seven** load-only codes. **Seven, not the six ANTS-3765 § 7 predicted** — `ambiguous_rematch` arrived with its § 2.6.1 at loop 3 and that § 7 line was never re-counted. Both specs are corrected; the miscount is recorded rather than silently fixed because it is the exact failure mode a vocabulary held in two documents produces, and it is why the codes are now enumerated under § 2.10's table instead of being referred to by count. **One claim of this spec was re-verified against source while implementing rather than taken on the word of the prose:** the id-less branch of `planFrom()` leaves `PlannedItem::idOrigin` **empty** (it sets `idAllocationOwed` and `provenance.id` only), so the load must supply one of `item.id_origin`'s three CHECKed values for an id it allocates. It writes `synthesised`, the only one of the three that is true of an id no source file contained; ANTS-3765 § 2.8 now states it. Nothing in this half changes. |
