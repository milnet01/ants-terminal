...
themselves cold-read**, which is what the cap means; and loop 3 found one
CRITICAL, generated by loop 2's own fix rather than present in the draft. A
fourth loop or a further split is available and is the user's call — the run
summary carries the numbers.
**Kind:** implement.
...
This part carries the umbrella's § 2.4 and its INV-4, plus two findings filed
by that run's loop-3 tail: **H3** (`id_strategy: "stable_prefix"` allocates no
counter id, so store-side allocation must not claim it — § 2.3) and **M6**
(`idHighWater()`'s `nullopt` case, `append_batch` contiguity and `dry_run` under
mutate-then-render — § 2.3, § 2.1). See the loop log's `0-split` row.
...
unmigrated branch, unchanged, and are deleted by the id § 5 names.
...
The sequence, step by step. Rows 5 and 8 are the two exits that are not
failures:
...
**The body append the table keeps referring to is one operation, and it is
simpler on the store path than in markdown.** `flip`-with-`note` and `annotate`
are wholly this: **the note is scrubbed (`rcScrubLeakedToolXml`) and capped at
`kRcMaxNoteChars` (4096) exactly as today, then appended to `item.body` after
its last non-blank line, separated by one `\n`.** Two things the markdown path
...
(`append` and `append_batch` create their item through `putItem()` and have
nothing to locate; `create_section` and `bundle_row` resolve a section, below.)
Those four locate a bullet by `id` or `headline` against the `BulletRecord`s the
read seam returned; the store writes take a primary key. The rule, in two steps,
because one does not cover the corpus:
...
// kind='table' exactly as addElement() does. Refuses when the row already
// at (section_id, position) is kind='item' — a different check from
// addElement()'s, which rejects its own `kind` ARGUMENT, but the same
// reason: putItem()/fileItem() stay the only ways an item is filed
// (INV-10, INV-20).
...
rule is unchanged; the comparison's right-hand side moves, and so does the code
at that site.
...
fills. The one caveat on `id` — that `rec.id` comes from the rendered head line and not
from the column, so it can differ from it — is § 2.2's, where the two-step
resolution that copes with it lives. It is correct behaviour (a caller locates
by the id it was shown), not something to "fix" by matching the column.
...
**On `append_batch` the refusal is per bullet, not per batch.** It lands in the
`skipped[]` array that op already returns, leaving every accepted bullet
applied — the same granularity § 2.4 gives `line_range` in `flip_batch`, and the
same one `append_batch` already uses for a bullet that fails validation (§ 2.3).
A whole-batch refusal would make one prose sentence in one bullet cost the other
nine.
...
**The store catches the loud mistake and not the quiet one.** Passing
`tv.lanes.value` — the raw pre-split capture, `"a, b"` — fails
`QJsonDocument::fromJson()` and is refused with "not JSON". Passing `"[]"` for a
key the body no longer carries is **not** refused: it is valid JSON and a valid
array, so it stores an empty array where the column should be NULL. The clearing
rule below is what keeps that case out of `setItemField()` entirely.
...
**A re-derivation clears a column ONLY when the body it replaced also yielded
that key.** Stated the short way — "clear whatever the new body does not
yield" — the rule destroys the state § 2.5's last paragraph explicitly blesses,
one op later and invisibly: an `append` supplies `layman: "X"` with a body
carrying no `Layman:` line (correct, the column holds `"X"`), then any later
`annotate` supplies no columns, re-derives all five, finds no `Layman:` in the
body, and clears the column — silently re-breaking the render gate § 2.6 exists
to make remediable. Every single-op test passes; the two-op sequence is where it
shows.

So the op asks `trailerValuesIn()` **twice** — once over the body it is
replacing and once over the body it is writing — and the four outcomes are:

| Old body yields | New body yields | Column |
|---|---|---|
...
- **INV-3** — **Counter allocation on a migrated project floors to both
  ...
  *Test:* `Inv3Allocation`, four cases — a store high-water above the corpus, a
  ...
  fails if the floor is dropped), an `append_batch` of **four bullets one of which fails validation**, asserting
  the three accepted ones take `first_id`, `first_id+1`, `first_id+2` — which is
  what exercises § 2.3's numbering-over-accepted rule rather than over input —
  and one high-water raise, and a
...
- **INV-4** — **An op that writes `body` leaves every trailer column the request
  ...
  *Breaks when:* the re-derivation is applied to all five unconditionally, so an
  ...
  cleared; a
  re-derivation clears a column the OLD body did not yield either, so a later
  `annotate` destroys a value an earlier `append` supplied as an argument — the
  failure no single-op test can reach; a
  ...
  refuses as "not JSON".
  *Test:* `Inv4BodyDerivesColumns`, five cases — `annotate` a `Layman:` line
  ...
  NULL, not `''`; an **`append` supplying `layman:` with a body carrying no
  `Layman:` line**, asserting the column holds the argument — the case that
  fails against an unconditional re-derivation; and **that same item then
  `annotate`d**, asserting the column *still* holds it — the two-op case that
  fails against a clearing rule keyed on the new body alone, and that no
  single-op case can reach. The first case's *second*
...
- **INV-7** — **`dry_run: true` commits nothing, on either path.** The store
  ...
  off `*outcome`.
- **INV-8** — **`bundle_row` edits the first `kind='table'` element's
  canonical-JSON payload, creating exactly one when the section has none.** *Breaks when:* the op appends a second `table`
  element to the section, or writes a rendered markdown row into a column
  ANTS-3756 INV-24 defines as canonical JSON. *Test:* `Inv8BundleRow`, three
...
`QByteArray` UTF-8 copy per file during staging. Against § 4's own byte figure
that is:

| Term | Peak |
|---|---|
| `contentOf` (all 3 files, UTF-16) | ~5.9 MiB (3,110,905 B × 2 ÷ 1024²) |
| staging `QByteArray` (one file at a time, UTF-8) | ~3.0 MiB |
| `itemOf` (1,844 items, bodies dominate) | ~5.9 MiB — the same text as one full roadmap, held as columns |
| **Peak per render walk** | **~14.8 MiB** |

Every cell is MiB, converted from the byte figure rather than relabelled from
the MB one — 3.1 **MB** of text is 5.9 **MiB** as UTF-16, not 6.2.
...
- **`docs/standards/mcp-error-codes.md` gains five codes and reuses one.**
  ...
  the standard** (re-verified 2026-08-04) — a pre-existing gap this spec inherits
  rather than causes. Filed as **ANTS-3823** rather than left as a disclaimer,
  since every other deferral here carries an id.
...
|---|---|---|---|---|
| 3 (cap) | 2026-08-04 | 2 (single doc, cold; same packet, extended with three source facts loop 2 had established by lookup) | 1 / 3 / 5 / 11 / 0 | **Converged by cap. 20 verified, 20 fixed, 5 dismissed.** Dimensions: dim 4×6, dim 6×4, dim 5×4, dim 2×3, dim 1×2, dim 10×2, dim 9×1, dim 15×1. **CRITICALs by loop: 2 → 1, and the one that remains is a fix-generated hazard neither earlier loop could have seen, because loop 2 created it.** Loop 2 scoped the re-derivation to "every key the request did not itself supply" and left the clearing rule keyed on the new body alone — so an `append` supplying `layman:` with a body carrying no `Layman:` line (the state loop 2 had just blessed) had that column **cleared by the next `annotate`**, silently re-breaking the render gate § 2.6 exists to make remediable. Invisible to every single-op test the spec prescribed. § 2.6 now asks `trailerValuesIn()` over the OLD body as well and clears only where the old body yielded the key; INV-4 gains the two-op case. Three HIGHs: the locator paragraph loop 2 added opened "**Six** of those rows take an `itemPk`" when it is **four** (`append` creates rather than locates, and the section ops were excluded 24 lines later — both lanes led on it); **the body append two of the eight ops consist entirely of was never defined** — a draft gap, now pinned as scrub + 4096-char cap + append after the last non-blank line of `item.body`, with the markdown path's indent-sniffing dropped because stored bodies carry no indent; and `body_shadowed`'s granularity on `append_batch` was unstated where § 2.4 had settled the identical question for `line_range`. **Five dismissed on verification, four of them repeats of packet limits rather than doc defects** — the ANTS-3793 § 1 citation (`grep` puts "26 call sites" at line 51; the packet only carried §§ 1–20, which is a packet defect and is why two loops raised it), the ANTS-3808 § 2.2.1 cite, `RoadmapDialog`'s absent write path, and the single-`flip` anchor refusal. **The fifth is the run's most interesting dismissal:** a lane argued `anchored` misclassifies a line-leading `Source:` because `rxSource` is un-anchored and would see a preceding space — true of an *indented* body, and `parseBullets()` builds `item.body` with `body.append(cont.trimmed())`, so stored bodies have no indent and `anchored` is exact for all five keys. The finding was wrong and pointed at an unstated precondition, which § 2.2 now states. Also fixed: the RAM table relabelled MB as MiB (3,110,905 B × 2 is 5.9 MiB, not 6.2 — peak ~14.8, not ~15.4); "rows 5 and 7 are the two exits that are not failures" when row 7 is `PublishFailed` and row 8 is the success; INV-8's headline excluded the create case its own test asserts; INV-8 said "two cases" over three (loop 2 added the third and left the count). `bad_op_combo`'s undocumented-code gap is filed as **ANTS-3823** rather than left as a disclaimer. **Loop 3's own fixes were not cold-read; that is what the cap means.** The document is **979 lines**, past the 934 the umbrella was split at — the size note is in the run summary, and it is a recommendation, not a gate failure. Lane spend 168k and 163k **cumulative** (first-turn input ~44k against the 60k per-turn budget). |
