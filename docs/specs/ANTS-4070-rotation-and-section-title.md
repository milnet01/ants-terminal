...
- **INV-6** — The archive path is derived, **project-root-relative**, and
  ...
  on its `##` line** — content before the first heading becomes a level-0 root
  section which stays on the live path, so a fixture carrying one can never
  reach zero live sections and the clause would be unreachable (§ 6).
  *Breaks when:* the path is
...
migrated file gains a **level-0 root section, slug `""`**, for whatever precedes
its first `##` heading — `roadmapmigrateload.cpp`'s `rebuildElements()` calls it
"the synthetic root … the same row the plan uses for content above the first
heading". A retitle creates no file, so INV-11's count is stable and
