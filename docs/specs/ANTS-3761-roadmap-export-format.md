...
**Amended 2026-08-03 by [ANTS-3796](ANTS-3796-section-record-completeness.md):**
§ 2.4's per-field table gains `section.position` and `section.source`; INV-2's
column list is now derived from `PRAGMA table_info` with an exhaustive
substitution map, because the hand-written list could not catch the column
ANTS-3797 found missing. Emission order, INV-1, INV-5 and INV-18 are untouched
by design; the three goldens were regenerated as a reviewed diff (exactly two
new keys on each of seven `section` records, no other byte).
**Kind:** implement.
...
| `section.position` | **never has no value** — ANTS-3796 § 2.1 makes the column `NOT NULL` with no default, so it has no absent state and is always emitted. Added 2026-08-03 by ANTS-3796, which owns the column |
| `section.source` | **always emitted**, as `null`. Added 2026-08-03 by ANTS-3797. Follows `section.parent` above rather than the omitted group because the `NULL` / `''` distinction is load-bearing (ANTS-3782 § 2.3): `null` is the live roadmap and `''` is an unplaceable path stored anyway, and an omit-when-absent rule would collapse them. Named `source` and not `source_path` — the record key drops the suffix the way `parent` drops `_id`; no collision with `item.source`, a different record type |
