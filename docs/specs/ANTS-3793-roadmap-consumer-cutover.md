...
|---|---|
| `id`, `status`, `kind`, `lanes`, `evidence`, `layman`, `source` | from the item columns, verbatim |
...
- **INV-5** — **`RoadmapParse` remains the only bullet grammar in `src/`,
  outside two enumerated exemptions.** *Breaks when:* the render or a consumer
  ...
  **Written as "one exemption" this invariant was red against today's tree, and
  ...
  So the invariant's exemption list is `roadmapparse.cpp` plus the two
  `rxBoldLayman` sites, named individually in the test, and any *fourth* site is
  a failure.
