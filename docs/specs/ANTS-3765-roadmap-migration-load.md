...
- **INV-16** — **A defaulted plan value never overwrites a non-empty stored value.** An item whose stored `kind` or `source` was set from a source that declared it keeps that value across a re-run whose source no longer declares it; the suppressed write raises a `field_conflict` note naming the id and the column, and `itemsUpdated` does not count the item when that was its only differing field. *Test:* `roadmap_migrate_load` **constructs both `MigrationPlan`s directly rather than parsing markdown**, for INV-1's reason applied to a different field: `kind` and `source` are trailer lines, so a source bullet that drops them also changes `body`, and a markdown-driven recipe would move two columns while claiming to move one — `itemsUpdated` would be 1 for a reason the invariant is not about. The plan is built with `body` byte-identical across both loads and only `provenance["kind"]` / `provenance["source"]` varying. Load one asserting `kind = fix` and `source = a-real-source` with provenance `asserted`; re-load with the imported defaults `implement` / `planned` and provenance `defaulted`; assert both stored values survive, that two `field_conflict` notes name them, and that `itemsUpdated == 0`. **The `itemsUpdated == 0` leg is the one that matters** and is why the recipe re-loads rather than seeding the store by hand: a rule that suppresses the write but still counts the item reports a change it did not make, so INV-2's idempotence claim becomes false on every corpus carrying an undeclared field — which is most of them. **A second leg asserts the rule does not block an ADD:** the same defaulted plan against an item whose stored `source` is empty writes `planned` and raises no note, because narrowing a re-run to adding information is the point of the rule and a version that suppressed both would pass the first leg alone. **That empty stored `source` is CONSTRUCTED, and no load path produces it** — import defaults `source` to the non-empty `planned`, so the first load must set it empty directly. This is INV-1's technique for INV-1's reason, and it is why both plans are built as structs: the state is unreachable through markdown, and a fixture-driven test could not build the case at all. *Breaks when:* the update path consults only `f.planEmpty` — the shipped shape, under which every defaulted field is a real value and overwrites unconditionally; or the check is placed after `setItemField()` rather than before it, which reports the suppression while having already performed the write.
