...
**Amended by ANTS-4500 (2026-09-21): an id-BEARING plan item that matches no stored id falls back to this key before being treated as new.** A stored row holds `ANTS-S0001` while the person filing that item by hand writes a conventional id, so § 2.6's match fails and the run produces a fresh insert plus an orphan; ANTS-4343 is evidence that path is used. The fallback runs as a THIRD pass, after the id-less pass and not interleaved with it, on that pass's own argument: this claim is exactly as weak as the one above and must not consume a row the sanctioned weak claim needs.

**On a match through that fallback the stored row takes the id the source carries**, through `RoadmapStore::reassignItemId()`, and the change is recorded in `history` under the field `id`. Keeping the stored `-S` id would make the next render overwrite the author's own edit in `ROADMAP.md`. Nothing cites a synthesised id by contract, so replacing it costs nothing the stored id was carrying; `element` and `history` key on `item_pk`, not on the id string, so they follow the row.

This is the one case that replaces an invented id. It does not weaken the rule above it: an id-less plan item is still matched first, and an id-bearing item still owns the row its id names.
...
1. **The prefix, and what a prefix even is.** An id's prefix is **the text before its final `-`**; an id with no `-` (`Sh4`, the legacy bold form) contributes none. That rule has to be stated because it is not the only candidate — over `ANTS-3765`, `Ts20-SP6` and `Sh4`, "before the first `-`", "before the last `-`" and "the leading alphabetic run" give three different answers. Then, **as of ANTS-3771, the project's DECLARED `id_format.prefix` from `.ants/project.json`, above all three terms below** — the store row records what *this store* has allocated, the declaration records what the *project* has decided, and where they disagree the project is right and the row is a stale artefact of an earlier migration. Without that rank a declaring project takes its declared prefix from `roadmap_log` and a sniffed or directory-derived one from a migration allocation, which is two id families in one store and the one outcome `UNIQUE (project_id, id_fold)` cannot even detect. It reaches the loader as `Options::idFormat`, supplied by the caller: `ProjectSettings` is in `ants_core_lib` and this library deliberately does not link it. Failing that: the project's existing `id_prefix` row when it has one; failing that, the **most frequent** prefix among the plan's `idOrigin == "parsed"` ids, ties broken by first appearance in document order; failing *that* — a project with no id-bearing items at all, the first-run case for several of the corpus's ten — the same fallback `roadmap_log` uses, **the uppercased first four characters of the project root's leaf directory**. (An earlier draft said "the caller's `exportSlug` uppercased, which is how `roadmap_log` derives one today". That was wrong on both halves: the verb's contract derives it from `caller_cwd`'s leaf directory, and `export_slug` is `[a-z0-9-]`-constrained, so uppercasing one can yield a hyphen-bearing prefix that step 2's suffix rule would then mis-split.)
...
**Amended by ANTS-4500 (2026-09-21): an allocated id is drawn from the SYNTHESIS namespace, not from the project's own counter space.** Steps 1 to 4 above describe the real prefix's counter, which allocation no longer reads. What changes:

- **The rendered text is `<prefix>-S<suffix zero-padded to four digits>`** — `ANTS-S0001`. Step 3's width rule is unchanged and its permanence argument applies here too. The `-S` infix is what lets a reader tell an invented id from an allocated one at sight, so an invented id is never a citation target.
- **The counter is a second `id_prefix` row keyed `<prefix>#S`.** `#` cannot occur in a declared prefix, so the key cannot collide with a real one; the table is keyed `(project_id, prefix)`, so this is a row rather than a column and needs no `kSchemaVersion` bump. `RoadmapStore::idPrefixFor()` ignores any prefix containing `#`.
- **The floor is three terms, on step 2's own argument that no term dominates**: the stored `<prefix>#S` counter, the maximum `S`-suffix among the project's stored ids (`RoadmapStore::maxSynthesisedId()`), and the maximum `S`-suffix among the plan's parsed ids. The third is the one that covers a store restored behind its file, which the first two cannot see.
- **Step 2's exclusions still hold for the real prefix, and by shape rather than by origin.** A synthesised id's suffix does not parse as an integer, so it raises neither of step 2's terms, and `maxAllocatedId()` globs `<prefix>-[0-9]*` and does not match it.
- **Migration also ensures the real prefix's `id_prefix` row exists**, at zero. Without it a wholly-synthesised project carries only a `#S` row and `idPrefixFor()` resolves nothing. What the project has actually allocated is `maxAllocatedId()`'s to report, and `allocationFloor()` reads both.

Items already holding an invented id keep it; the change is to new synthesis only. Contract: `docs/specs/ANTS-4500-synthesised-id-namespace.md`.
...
**Memory.** One plan at a time, and ANTS-3757 § 4 already budgets the plan itself; this half adds the `Bulk` connection's **16 MiB** page cache (ANTS-3756 § 2.5, `kBulkCacheKiB`) plus one `readItem()` result at a time. It does build one in-memory map of the project's existing items, `id_fold` to row key (`byIdFold` in `roadmapmigrateload.cpp`), filled from `listItems()`; that map grows with the project's item count. The per-project item counts are ANTS-3757 § 1.1's and are not restated here — that section is the single home for every corpus figure precisely because the same count drifting between documents was a finding in two of its review loops. Ten projects migrated through one connection cost one 16 MiB cache, not ten, which is the second reason § 2.2 takes an open store rather than opening its own.
