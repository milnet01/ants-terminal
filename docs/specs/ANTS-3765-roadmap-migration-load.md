...
- **Rule "defaulted does not overwrite":** a plan value whose provenance is `defaulted` does not overwrite a non-empty stored value. It raises the same `field_conflict` note as its sibling above, and for the same reason: the source did not say this. A defaulted value is import's own guess — `kind` to `implement`, `source` to `planned` (roadmap-format.md § 3.5.3's own defaults, ANTS-4065 § 2.3) — so writing one over a stored value replaces evidence with invention. **The two rules are disjoint, not overlapping:** every defaulted value is non-empty, so a field never satisfies both, and `body`'s exception does not reach here because provenance is only ever recorded for `status`, `id`, `kind` and `source` (ANTS-3757 § 2.7–2.9) and `body` is not among them. **Where the stored value IS empty the default is written**, which is what keeps a first migration whole: the rule narrows a re-run to ADDING information, and that is precisely what makes it safe to re-run without a backup. **`status` is the field to check this against, and it is safe:** only a `pass-headings` block missing its `- **Status**:` line defaults status, while `ants-v1` and `github-task-list` transcribe roadmap-format.md § 3.5's markers and are therefore `asserted` — so no format in the corpus can have its status frozen by this rule. Without it a re-migration of this project would carry 384 defaulted `source` values over rows that already held one (measured 2026-08-19 by `roadmap_migrate dry_run`, reported as `defaulted_fields: {source: 384, kind: 1}` against 598 `items_updated`).
