...
**Sixteen methods and one struct** — nine writers and seven readers — each traceable to the section that needs it; § 2.3 adds four more, so **twenty** methods land on ANTS-3756 in total (§ 7). All are additions rather than corrections, and the readers are as load-bearing as the writers: a re-run cannot decide anything it cannot first read, and the shipped surface has no reader at all beyond `db()`.
