...
- **[ANTS-4622](ANTS-4622-cross-session-mailbox.md) amends this document rather than merely citing it** (2026-08-22). The schema gains its **tenth table**, `message` — the cross-session mailbox — plus two indexes, `idx_message_inbox (to_project_id, acked_at)` and `idx_message_from (from_project_id)`. **Two indexes because § 4's own FK rule requires it**: SQLite does not auto-index foreign keys, and `message` names TWO projects, so both `from_project_id` and `to_project_id` are FK columns leading nothing — `relationship` is the precedent, carrying `idx_rel_src` AND `idx_rel_dst`. `kSchemaVersion` moves **2 → 3**, the second bump, and unlike ANTS-3815's it adds no COLUMN: a whole new table is issued identically by `createSchema()` and by the rung, so both cite ONE shared constant (`kMessageTableDdl` and the two index constants, in `roadmapstore.cpp`'s anonymous namespace) rather than keeping two copies in step. **ANTS-3781 INV-8 therefore holds by construction here**, and this bump is the first that makes it non-vacuous — at one rung there was nothing to climb. `deregisterProject()` grows a tenth step and `DeregisterCounts` a ninth counter, `messages`; that step clears **both ends** and binds the project id twice, for the reason `relationship`'s step already did. **Four methods join this surface**: `sendMessage`, `inboxFor`, `ackMessage` and `pruneAckedMail`, plus two resolvers the mailbox needed and nothing else exposed — `projectIdForSlug()` and `projectIdForRoot()`, the latter deliberately NOT `registerProject()`, which is upsert-shaped and would silently register any root a verb was handed into a machine-global store. Timestamps are PARAMETERS on all four, matching `appendHistory()`'s `changedAt`: the store does not read the clock, which is what lets a test construct a row on each side of a 30-day TTL. **No invariant here changes** — the new table is outside every existing `UNIQUE`, references only `project`, and is named by no invariant in *this* document; ANTS-4622's own INV-1..INV-10 constrain it.
